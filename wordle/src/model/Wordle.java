...
/**
 * This class is the main logic/model in the game.
 *
 * @author UBIS
 */
public class Wordle extends MyObservable implements Serializable {
  ...
  /**
   * The constructor creates a library of words of length 5, generates a word for the user to guess,
   * and initializes the number of attempts left. It also initializes the attempts to empty strings
   * (the reason we do this is because we're going to concatenation on the attempts so they need to
   * be already initialized). The constructor also initializes the index of the current attempt.
   */
  public Wordle() {
    hasWon = false;
    hasLost = false;
  ...
  /**
   * This constructor does the same thing as the previous constructor except for generating a random
   * word. This constructor allows us to choose the correct guess and will be used for testing.
   */
  ...
  /**
   * @param keyContent: the content of the key pressed/clicked This method grows our current
   *     attempt.
   */
  public void addAttempt(String keyContent) {
    ...
    // This explains the placement of notify() in this block of code.
      ...
      attempts[currentAttemptIndex].setAttempt(
          attempts[currentAttemptIndex].getAttempt().toLowerCase());
      ...
      if (exists()) {
        ...
        if (attempts[currentAttemptIndex].getAttempt().equals(correctWord)) {
          hasWon = true;
        } else if (attemptsLeft == 0) hasLost = true;
        notifyObservers(this);
        currentAttemptIndex += 1;
        lockedIn = false;
      } else notifyObservers(this);
      // after notifying the observer, we move on to the next attempt
      // and go back to a state where lockedIn is false;
    } else if (!keyContent.equals("enter")
        && !keyContent.equals("backspace")
  ...
  /**
   * @return the current attempt. This method is necessary for when we update the keyboard view.
   */
  ...
  /**
   * @return feedback received on the attempt. (feedback is explained better in Attempt class) This
   *     method will be called in the views because it tells us which tiles to color in green, which
   *     tiles to color in yellow, and which tiles to color in black.
   */
  ...
  /**
   * @return a boolean indicating if the current attempt is locked in. In other words, if enter was
   *     clicked/pressed.
   */
  ...
  /**
   * @return the most recent letter entered. This method is necessary when updating AttemptView.
   */
  ...
  /**
   * @return the array containing the attempts. This method is necessary for persistence (when we
