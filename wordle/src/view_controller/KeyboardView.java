...
/**
 * This class represents the view for the keyboard in the Wordle game. It extends the VBox class and
 * implements the MyObserver interface.
 */
public class KeyboardView extends VBox implements MyObserver {
      ...
      for (int j = 0; j < attempts[i].getAttempt().length(); j++) {
        for (Button button : keys.values()) {
          if (button
              .getText()
              .toLowerCase()
              .equals(String.valueOf(attempts[i].getAttempt().charAt(j)))) {
            if (positions[j] == 0)
  ...
  public void setLight() {
    ...
    for (Button button : keys.values()) {
      ...
      if (color.equals("black")) button.setStyle(null);
    }
  }
}
