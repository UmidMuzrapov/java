/** Umidjon Muzrapov Salim Choura Jose Bernardo Montano Ivan Denisovich Akinfiev */
...
/**
 * AttemptView extends BordePane and implements MyObserver, it has instance variables that keep
 * track of username, gridPane, funcPane, welcomeLabel, myButtons, currentButtonIndex, and isOver.
 * It displays the user's attempts in the Wordle Game
 */
public class AttemptView extends BorderPane implements MyObserver {
  ...
  /**
   * The constructor for AttemptView receives a Wordle game, and initializes some of the instance
   * variables such as: myButtons, gridPane. It creates 30 buttons with their respective
   * functionality, finally the constructor calls loadGame with the Wordle object
   *
   * @param game
   */
  ...
  /**
   * shakeCurrentButton method shakes a specific button based on index, it makes sure that the game
   * is notOver and that the button is not out of range
   *
  ...
  private void shakeCurrentButton(int index) {
    ...
    TranslateTransition shakeTransitionOne =
        new TranslateTransition(Duration.seconds(0.25), button);
    shakeTransitionOne.setByX(10);
    shakeTransitionOne.play();
    shakeTransitionOne.setOnFinished(
        (event) -> {
          TranslateTransition shakeTransitionTwo =
              new TranslateTransition(Duration.seconds(0.25), button);
          shakeTransitionTwo.setByX(-20);
          shakeTransitionTwo.play();
          shakeTransitionTwo.setOnFinished(
              (eventTwo) -> {
                TranslateTransition shakeTransitionThree =
                    new TranslateTransition(Duration.seconds(0.25), button);
  ...
  /**
   * loadGame method load the Wordle game into the GridPane, and retrieves the attempt by setting
   * the text and style for each corresponding letter and button
   *
    ...
    for (int i = 0; i < attempts.length; i++) {
      ...
      for (int j = 0; j < attempts[i].size(); j++) {
        ...
        myButtons
            ...
            .setText(attempts[i].getAttempt().substring(j, j + 1).toUpperCase());
        if (positions[j] == 0)
          myButtons
              .get(currentButtonIndex)
              .setStyle(
                  "-fx-text-fill:white;-fx-opacity: 1; -fx-background-color:rgb(108,169,101)");
        if (positions[j] == 1)
          myButtons
              .get(currentButtonIndex)
              .setStyle(
                  "-fx-text-fill:white;-fx-opacity: 1; -fx-background-color:rgb(120,124,127)");
        if (positions[j] == 2)
          myButtons
              .get(currentButtonIndex)
  ...
  /**
   * The method flipCurrentButton flips the a button at the current index, which will display the
   * button with the letter and background color
   *
  ...
  private void flipCurrentButton(int index, String letter, String color) {
    ...
    rotateTransition.setOnFinished(
        event -> {
          ...
          newButton.setStyle(
              "-fx-background-color:" + color + "; -fx-text-fill: white; -fx-opacity: 1;");
  ...
  /**
   * update method is call whenever the observable object is changed, and it updates the view based
   * on the state of the Wordle game
   *
  ...
  public void update(MyObservable observable) {
    ...
    if ((currentButtonIndex) < 30) myButtons.get(currentButtonIndex).setText(lastLetter);
    if (game.hasWon()) {
  ...
  /** setDark method will change the CSS properties of the components to make it dark mode */
  public void setDark() {
    ...
    for (int i = 0; i < 30; i++) {
      if (i >= (currentButtonIndex / 5) * 5) {
        myButtons
            .get(i)
            .setStyle("-fx-background-color: black; -fx-opacity: 1; -fx-text-fill: white;");
      }
    }
  }

  /** setLight method will change the CSS properties of the components to make it light mode */
