 ...
 * ConnectFour
 *The program allows two users to play ConnectFour game.
 *The target values can be from 3 to 6 for win. 
 *The outcome of the game can be draw or win.
 *The user can play game again if they want.
 */
...
public class ConnectFour 
	...
	/**
	 * main is main logic of the game.
	 * It takes user information, switches the users, 
	 * checks for the wins,and repeats the game if prompted.
	...
	/**
	 * getTargetValue method asks the user for the winning token number
	 *  and then it verifies the input.
	...
	/**
	 * takeUserInformation takes in the user information, including 
	 * their names and color of choice.
	...
	{	
		...
		{
			System.out.print("It seems like"
	...
	/**
	 * colorMatches makes sure that the used has chosen one
	...
	static boolean colorMatches(String colorPerson1)
	{
		ArrayList<String> colors=
				new ArrayList<>(Arrays.asList("Y","R"));
	...
	/**
	 * buildGrid replaces null values in the 
	 * original 2-d string to empty spaces.
	...
	/**
	 * playGame asks for the column to move, processes the move
	 * checks for the win.
	 * @param grid 2-d array with the strings.
	 ...
	 */
	static boolean playGame(String[][] grid, String[]
			currentPlayer, int winningTokenNumber )
	{
	...
	/**
	 * columnChosenValid checks if the entered 
	 * value is in valid and in the right range.
	 ...
	 */
	static void processMove(String[][] grid,
			String[] currentPlayer, int columnChosen)
	{
		int row=6;
	...
	/**
	 * checkWin checks the input horizontally, vertically,
	 * in both diagonals only in the locations where the player appears.
	 * @param currentPlayer string array that represents the current player 
	 * @param grid 2-d array with the strings.
	 * @param winningTokenNumber integer that is the target value
	 * @return only true if the win is found.
	 */
	static boolean checkWin(String[]currentPlayer,
			String[][] grid, int winningTokenNumber)
	{
	...
	/**
	 * getLocations finds the locations of the current player to
	 * check for the win.
	 * @param currentPlayer string array that represents the current player 
	 * @param grid 2-d array with the strings.
	 ...
	 * @param location the integer string
	 *  that shows the r,w where the player appears
	 * @param grid 2-d array with the strings.
	 * @param winningTokenNumber integer that is the target value.
	 * @param currentPlayer string array that represents the current player 
	 * @return only true if the win is found.
	 */
	static boolean checkHorizontal(int[] location,
			String[][]grid, int winningTokenNumber, String[] currentPlayer) 
	{
	 ...
	 * @return only true if the win is found.
	 */
	static boolean checkVertical(int[] location,
			String[][]grid, int winningTokenNumber, String[] currentPlayer)
	{
		int columnStart=location[1];
		int rowStart=location[0];
	 ...
	 * @return only true if the win is found.
	 */
	static boolean checkDiagonalSouthEast(int[] location,
			String[][] grid,int winningTokenNumber, String[] currentPlayer)
	{
		int columnStart=location[1];
		int rowStart=location[0];
	 ...
	 * @return only true if the win is found.
	 */
	static boolean checkDiagonalNorthWest(int[] location,
			String[][] grid,int winningTokenNumber, String[] currentPlayer)
	{
		int columnStart=location[1];
		int rowStart=location[0];
	 ...
	 */
	static String[] switchPlayers(String[] player1,
			String[] player2, String[]currentPlayer)
	{
	 ...
	 * Based on their response, it restarts the new game, or
	 * exits.
