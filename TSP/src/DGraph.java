...
/**
 * This class represents the directed graph. It contains methods to solve the TSP.
 *
...
public class DGraph {
  ...
  /** This method prints out the graph, printing out the vertex and its edges. */
  ...
  /**
   * This methods implements heuristic algorithm to find the approximate Hamiltonian cycle.
   *
  ...
  public void useHeuristic(int startVertex, boolean showTime) {
    ...
    showResults(showTime, startTime, endTime, totalCost, path, "heuristic");
  }
  ...
  public void useBacktrack(int start, boolean showTime) {
    ...
    path.add(start);
    double totalCost = recursiveSub(start, 0, path, visited, Double.MAX_VALUE, minPath);
  ...
  /**
   * This is my first improvement on heuristic method. Please read readme for more details.
   *
   * @param showTime boolean that indicates to show time or not.
   */
  public void mineHeuristic(int start, boolean showTime) {
    ...
    while (path.size() != vertices.size()) {
      for (int i = 0; i < adjacentVertices.get(currentVertex).size(); i++) {
        int potentialVertex = adjacentVertices.get(currentVertex).get(i).getNeighbour();

        if (!path.contains(potentialVertex)) {
          path.add(potentialVertex);
          totalWeight = totalWeight + adjacentVertices.get(currentVertex).get(i).getWeight();
    ...
    long endTime = System.nanoTime();
    showResults(showTime, startTime, endTime, totalWeight, path, "mineHeuristic");
  }
  ...
  public void mineBacktrack(int start, boolean showTime) {
    ...
    ArrayList<Integer> minPath = new ArrayList<Integer>();
    path.add(start);
    double totalCost = recursiveSubMine(start, 0, path, visited, Double.MAX_VALUE, minPath);

    long endTime = System.nanoTime();
    showResults(showTime, startTime, endTime, totalCost, minPath, "mineBacktrack");
  }

  /**
   * This is my second improvement on heuristic method that works without pre-sorting if necessary.
   *
   * @param showTime boolean that indicates to show time or not.
   */
  public void mineHeuristicNoStorting(int start, boolean showTime) {
    ...
    long endTime = System.nanoTime();
    showResults(showTime, startTime, endTime, totalWeight, path, "mineHeuristickNoStorting");
  }
    ...
    if (path.size() == vertices.size()) {
      ...
      for (Edge edge : adjacentVertices.get(currentVertex)) {
        if (!visited[edge.getNeighbour()]) {
          ...
          totalCost =
              recursiveSub(edge.getNeighbour(), currentCost, path, visited, totalCost, minPath);
  ...
  /**
   * This is an auxiliary recursive method for the backtrack method improvement.
   *
   * @param currentVertex current vertex
   * @param currentCost the total cost of the path so far.
   * @param path the list of visited vertices.
   ...
   * @return
   */
  private double recursiveSubMine(
      int currentVertex,
      double currentCost,
      ArrayList<Integer> path,
      boolean[] visited,
    ...
    if (path.size() == vertices.size()) {
      ...
      for (Edge edge : getEdges(currentVertex)) {
        if (!visited[edge.getNeighbour()]) {
          ...
          currentCost = currentCost + edge.getWeight();
          totalCost =
              recursiveSubMine(edge.getNeighbour(), currentCost, path, visited, totalCost, minPath);
          path.remove(path.size() - 1);
          currentCost = currentCost - edge.getWeight();
          visited[edge.getNeighbour()] = false;
        }
  ...
  /**
   * This helper function finds the minimal total cost for backtrack method, considering the back
   * path to vertex 1.
   *
   ...
   * @return total cost.
   */
  private double checkMinPathBacktrack(
      double currentCost, ArrayList<Integer> path, double totalCost, ArrayList<Integer> minPath) {
    currentCost = findPathBack(path, currentCost);

    if (currentCost < totalCost
        || (currentCost == totalCost && pathHasLowerVertex(path, minPath))) {
      minPath.clear();
  ...
  /**
   * This helped method finds the total cost for heuristic method, including the path back to vertex
   * 1.
   *
   * @param path the list of visited vertices.
  ...
  /**
   * This method prints out the results of the search, configuring it based on the user input.
   *
   * @param showTime boolean that indicates to show time or not.
  ...
  private void showResults(
    ...
    if (!showTime) {
      System.out.println(
          "cost = " + String.format("%.1f", totalCost) + ", visitOrder = " + path.toString());
    }
  ...
  /**
   * This method sorts the adjacency list of edges. In read me file, I will consider the cases with
   * and without sorting.
   *
  ...
  /**
   * This method creates the adjacency list from the the list of edges (the edge is not an Edge
   * object.).
   *
   * @param edges represents the edges of the graph.
   */
