/**
 * @author: Umid Muzrapov Class CheapHouses allows the users to input file name and house price. It
 *     reads data from a CSV file, and plots the the houses whose cost is below the price specified
...
public class CheapHouses {
  ...
  /**
   * main is program's starting point and calls another method to set up the GUI.
   *
  ...
  /**
   * plotData plots the houses below the given price as filled circles in PlotPanel.
   *
   * @param g Graphics object that is used to draw shapes.
   * @param houseMap HashMap<String, ArrayList<Double>> maps house addresses, and their latitude,
   *     longitude, price.
   * @throws NumberFormatException Exception thrown if non-numeric value is given for the price.
   */
  static void plotData(Graphics g, HashMap<String, ArrayList<Double>> houseMap)
    ...
    for (ArrayList<Double> priceAndCoordinates : houseMap.values()) {
      if (Double.compare(priceAndCoordinates.get(0), Double.parseDouble(priceTextField.getText()))
          < 0) {
        double latitude = priceAndCoordinates.get(1);
        double longitude = priceAndCoordinates.get(2);
        double[] xyCoordinates = convertLatitudeLongitude(latitude, longitude, maxAndMinLatAndLong);
  ...
  /**
   * buildHouseMap builds a HashMap that maps address and latitude, longitude, price.
   *
   * @param inputFile Scanner objects used to read a line.
   * @return houseMap HashMap<String, ArrayList<Double>> maps house addresses, and their latitude,
   *     longitude, price.
  ...
  static HashMap<String, ArrayList<Double>> buildHouseMap(Scanner inputFile)
    ...
    HashMap<String, ArrayList<Double>> houseMap = new HashMap<String, ArrayList<Double>>();
    String address;
  ...
  /**
   * convertLatitudeLongitude converts given latitude and longitude to x and y values. Latitudes and
   * longitudes can be any rational numbers.
   *
   * @param latitude double that represent latitude
   * @param longitude double that represent longitude
   * @param maxAndMinLatAndLong double array that contains the max and min latitudes and longitudes.
   * @return xyCoordinates double array that contains x and y coordinates.
   */
  ...
  /**
   * findMaxAndMinLatAndLong finds the max and min values of latitudes and longitudes.
   *
   * @param houseMap HashMap<String, ArrayList<Double>> maps house addresses, and their latitude,
   *     longitude, price.
   * @return maxAndMinLatAndLong double array that contains the max and min latitudes and
   *     longitudes.
   */
  static double[] findMaxAndMinLatAndLong(HashMap<String, ArrayList<Double>> houseMap) {
    ...
    double[] maxAndMinLatAndLong = {minLatitude, maxLatitude, minLongitude, maxLongitude};
    return maxAndMinLatAndLong;
  }

  ///////////// GUI Methods and Classes here/////////////

  /** createAndShowGui sets up the GUI. Initially, plotPanel is not there. */
  ...
  /**
   * buildControlPanle creates the control panel and adds the necessary elements to it.
   *
  ...
  /**
   * Class PlotPanle inherits from JPanel. It represents a plotPanel that displays data from csv
   * file.
   */
  static class PlotPanel extends JPanel {
    /** This is a constructor. It sets up the size and location of the panel. */
    public PlotPanel() {
      setSize(800, 640);
      setLocation(0, 0);
    }
    ...
    /**
     * paintComponent is main logic of program. It reads the file, builds the map and plots the
     * Data.
     *
     * @param g Graphics object that is used to draw shapes.
     */
    public void paintComponent(Graphics g) {
      try {
        ...
        g.drawString(
            "The string or nothing was given," + " where the number was expected.", 280, 320);
        g.drawString(
            "Check your input and the values" + " in file for price, lat and long.", 280, 350);
      }
    }
  }

  /** Class PlotButtionListener handles an event raised by button-click. */
  private static class PlotButtonListener implements ActionListener {
    /**
     * actionPerformed creates the plotPanel and add it to the mainPanle when the button is clicked.
     *
