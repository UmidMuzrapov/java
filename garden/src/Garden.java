/**
 * @author: Umid Muzrapov Garden class represent a garden, where plants can be removed and planted.
...
public class Garden {
  ...
  /**
   * The constructor initializes the garden of the given rows and columns.
   *
  ...
  /**
   * remove(Plant plantType) removes all plants that belong to the given class type.
   *
  ...
  /**
   * remove (String specificPlantName) removes all plants whose plant name (banana, sunflower,
   * etc.,) matches specificPlantName.
   *
  ...
  /**
   * remove(Plant plantClass, int numPlantRow, int indexElementInInnerArray) tries to remove the
   * plant of the specified class in the given coordinates.
   *
  ...
  public void remove(Plant plantClass, int numPlantRow, int indexElementInInnerArray) {
    ...
    if (!indexOKAndNotNull(numPlantRow, indexElementInInnerArray)
        || !this.gardenArray[numPlantRow][indexElementInInnerArray]
            .getClass()
            .equals(plantClass.getClass())) {
      System.out.println(
          "Can't "
              + chooseVerb(plantClass, numPlantRow, indexElementInInnerArray).toLowerCase()
              + " there.\n");
    } else {
      this.gardenArray[numPlantRow][indexElementInInnerArray] = null;
    }
    ...
    for (int numPlantRow = 0; numPlantRow < numInnerArrays; numPlantRow++) {
      for (int index = 0; index < 5; index++) {
        for (int indexElementInInnerArray = 0;
          ...
          if (this.gardenArray[numPlantRow][indexElementInInnerArray] != null) {
            System.out.print(
                this.gardenArray[numPlantRow][indexElementInInnerArray].plantArray[index]);
          } else {
            System.out.print(".....");
          }
        }
  ...
  /**
   * growMyPlantType grows the plants belonging to plantType class the given times.
   *
  ...
  public void growMyPlantType(int timesToGrow, String plantType) {
    ...
    for (int numPlantRow = 0; numPlantRow < numInnerArrays; numPlantRow++) {
      for (int indexElementInInnerArray = 0;
          ...
          indexElementInInnerArray++) {
        Plant plant = this.gardenArray[numPlantRow][indexElementInInnerArray];
        if (plant != null && plant.getClass().equals(stringToClassMap.get(plantType).getClass())) {
  ...
  /**
   * growPlantInPlot grow the plant in specified coordinates if the plant there is planted.
   *
   * @param timesToGrow int
   * @param numPlantRow int
   * @param indexElementInInnerArray int
   */
  public void growPlantInPlot(int timesToGrow, int numPlantRow, int indexElementInInnerArray) {
    if (!indexOKAndNotNull(numPlantRow, indexElementInInnerArray)) {
  ...
  /**
   * chooseVerb chooses the right verb based on the plant class for the remove method.
   *
   * @param plantClass Plant
   * @param numPlantRow int
   * @param indexElementInInnerArray int
   * @return String the remove verb.
   */
  private String chooseVerb(Plant plantClass, int numPlantRow, int indexElementInInnerArray) {
  ...
  /**
   * indesOKAdnNotNull makes sure that the indices are good, and the plant exists there.
   *
   * @param numPlantRow
   * @param indexElementInInnerArray
   * @return boolean true if indices are good and there is a plant.
   */
  public boolean indexOKAndNotNull(int numPlantRow, int indexElementInInnerArray) {
    if (numPlantRow >= this.numInnerArrays
        || indexElementInInnerArray >= this.numElemenetInInnerArray
        || this.gardenArray[numPlantRow][indexElementInInnerArray] == null) return false;
    else return true;
  }

  /**
   * getStringToClassMap matches the removal verb to the the right class.
   *
